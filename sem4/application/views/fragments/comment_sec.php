    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            function longpolling(rows){
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>" + "comments/longpolling",
                    dataType: 'json',
                    data: {
                        rows: rows,
                        site: site
            ...
            $('#commento').on('click', '.delete-comment', function(){
                ...
                jQuery.ajax({
                    url: "<?php echo base_url(); ?>" + "comments/delete/" + comment_id + "/"+ site,
                    dataType: 'json',
                    success: function(res) {
                        if (res) {
...
-->
    <script src="<?php echo base_url(); ?>resource//js/comments.js"></script>
        <?php 
    ...
    <div id="commento">
        ...
        if($this->session->userdata('logged_in')){
            ...
            <br />
            <input id="current-user" value="' . $this->session->userdata('username') . '" type="hidden">
            <button id="submit-comment" type="submit" class="btn btn-default" name="site" value="' . $recipe->title . '">Submit</button>
        </form>';
        }else{
            echo '<form>
            <label for="comment">Comment:</label>
            ...
            <br />
            <button id="submit-comment" class="btn btn-default" name="site" disabled="disabled" name="site" value="' . $recipe->title . '">Submit</button>
        </form>';
        }
